...
# assistant_discourse_with_external_capture_driving.sh
# Handles the presentation and logging of assistant discourse messages, with
# flexibility for different language preferences and configuration settings. It
# lacks the libcommbase routines capture_mute.sh and capture_unmute.sh, so the
# capture is managed externally.
assistant_discourse_with_external_capture_driving() {
  ...
  if [ "$AUDIBLE_ASSISTANT_LOGGING_ON" = "True" ]; then
    ...
    (tmux select-window -t 1 && tmux select-pane -t "$pane_number" && tmux send-keys " clear; echo \"[$timestamp]\" \"$origin:\" \"$log_severity_level_1:\" \"$ASSISTANT_NAME_IN_CHAT_PANE\" \"$discourse\" >> \"$COMMBASE_APP_DIR/$CHAT_LOG_FILE\"; echo \"$discourse\" | $TTS_ENGINE_STRING" C-m && sleep "$time");
  else
...
(assistant_discourse_with_external_capture_driving "$pane_number" "$i18n_number" "$origin" "$log_severity_level_1" "$discourse_key")
