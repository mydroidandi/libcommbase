...
restart_stt_engine() {
  ...
  if [ "$STT_ENGINE_PATH" = "$COMMBASE_APP_DIR/bundles/commbase-stt-whisper-reactive-p/commbase_stt_whisper_reactive_p.py" ]; then
    ...
    if [ "$RECORDER_TRANSMITTER_FILE" = "$CUSTOM_RECORDER_TRANSMITTER_FILE" ]; then
      ...
      (tmux select-window -t 1 && tmux select-pane -t 7 && tmux send-keys " clear; bash $COMMBASE_APP_DIR/$CUSTOM_RECORDER_TRANSMITTER_FILE" C-m && sleep "$time");
...
# Call restart_stt_engine if the script is run directly (not sourced)
